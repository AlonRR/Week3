...
let zipCode=users.filter(user=> user.address.zipcode[0]===`5`)

//ex3
// Modify your code from Exercise 2 so that your resulting array only has the IDs of the matching users.
//  You should chain your filter with a map.
//  Ultimately, you should just have this array: [3, 4, 7]
let zipCode=users.filter(user=> user.address.zipcode[0]===`5`).map(user=> user.id)
...
let namesThatStartWithC = users.filter(user=> user.name[0]===`C`).map(user=> user.name)
...
console.log(users.filter(user=> user.address.suite === `Apt. 950`).map(user=>user.company.name).toString())
*/
//ex7
